...
class MailController extends Controller
{
    ...
    public function send(Request $request): RedirectResponse
    {
        ...
        if ($sendMode === 'test') {
            ...
            Mail::to($testRecipient)->send(new CampaignBroadcastMail($validated['subject'], $validated['message']));
        ...
        foreach ($uniqueRecipients->chunk(100) as $batch) {
            foreach ($batch as $recipient) {
                Mail::to($recipient)->send(new CampaignBroadcastMail($validated['subject'], $validated['message']));
                $sentCount++;
            }
        }
