...
<section class="container mx-auto w-full max-w-5xl p-4 md:p-6">
    <div class="mb-5 rounded-xl bg-white p-4 shadow md:p-5">
        ...
        <p class="mt-3 text-xs font-bold uppercase tracking-widest text-green-700">New article</p>
        <h1 class="mt-1 text-lg font-bold text-gray-950 md:text-xl">Create blog post</h1>
        <p class="mt-1 text-sm text-gray-600">Write, format, and choose whether to publish now or keep the post archived.</p>
    </div>
    ...
    <form method="post" action="{{ route('admin.blogs.store') }}" class="space-y-7 rounded-xl bg-white p-4 shadow md:p-6">
        @csrf
