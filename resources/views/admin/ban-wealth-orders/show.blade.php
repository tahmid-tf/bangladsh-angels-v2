...
<div class="max-w-6xl mx-auto">
    ...
    <div class="grid lg:grid-cols-[1fr_340px] gap-6">
        ...
        <aside class="space-y-5">
            ...
            <form method="POST" action="{{ route('admin.ban-wealth-orders.update', $banWealthOrder) }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">@csrf @method('PATCH')<h2 class="font-bold text-lg mb-4">Review decision</h2><label class="block text-sm font-semibold mb-2">Status<select name="status" class="mt-1 w-full rounded-lg border-gray-300">@foreach (\App\Models\BanWealthOrder::STATUSES as $status)<option value="{{ $status }}" @selected(old('status',$banWealthOrder->status)===$status)>{{ $status === 'pending' ? 'Pending review' : ucfirst($status) }}</option>@endforeach</select></label>@error('status')<p class="text-red-600 text-xs">{{ $message }}</p>@enderror<label class="block text-sm font-semibold mt-4">Review note<textarea name="review_note" rows="5" class="mt-1 w-full rounded-lg border-gray-300" placeholder="Shown to the investor">{{ old('review_note',$banWealthOrder->review_note) }}</textarea></label>@error('review_note')<p class="text-red-600 text-xs">{{ $message }}</p>@enderror<button class="w-full mt-4 px-4 py-3 rounded-lg bg-[#0a5554] text-white font-bold">Save decision</button>@if($banWealthOrder->reviewer)<p class="text-xs text-gray-500 mt-3">Last reviewed by {{ $banWealthOrder->reviewer->name }} on {{ $banWealthOrder->reviewed_at?->format('d M Y, g:i A') }}</p>@endif</form>
        </aside>
    </div>
</div>
@endsection
