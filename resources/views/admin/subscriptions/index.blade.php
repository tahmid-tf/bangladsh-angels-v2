    ...
    <table class="min-w-full border-collapse border border-gray-200 text-left text-sm">
      <thead>
          <tr class="bg-gray-100">
              ...
              <th class="px-6 py-4 font-medium text-gray-600">Requested</th>
          ...
          @foreach ($subscriptions as $subscription)
            @if ($subscription->user->id!==auth()->user()->id && !$subscription->user->isFree())
              <tr class="border-t">
                  ...
                  <td class="px-6 py-4">{{ $subscription->user->designation ?? '-' }}</td>
                  <td class="px-6 py-4">{{ $subscription->user->company_name ?? '-' }}</td>
                  <td class="px-6 py-4">{{ $subscription->user->phone ?? '-' }}</td>
                  <td class="px-6 py-4 text-sm text-gray-600">{{ $subscription->user->email }}</td>
                  ...
                  <td class="px-6 py-4 text-sm text-gray-600 text-center">
                    <a href="{{route('member.edit',$subscription->user->id)}}">
                  ...
                  <form id="status-form" method="POST" action="{{route('update.account.status',$subscription->user->id)}}">
                      @csrf
                      @method('PATCH')
                      <input type="hidden" name="user_id" id="user_id" value="{{$subscription->user->id}}">
