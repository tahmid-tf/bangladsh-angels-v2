...
@section('page_title', 'Edit team member | Dashboard')
...
<div class="container w-full p-6 max-w-3xl">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold mt-4 text-gray-900">Edit: {{ $member->name }}</h1>
        <p class="text-sm text-gray-600 mt-2">
            @if ($member->section === \App\Models\TeamMember::SECTION_MANAGEMENT)
                Team &amp; Management
            @else
                Governing Board
            @endif
    ...
    <form method="post" action="{{ route('admin.team-members.update', $member) }}" enctype="multipart/form-data" class="bg-white rounded-lg shadow p-6 space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-sm font-semibold text-gray-700 mb-1">Name</label>
            <input type="text" name="name" id="name" value="{{ old('name', $member->name) }}" required maxlength="255"
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $member->title) }}" required maxlength="255"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-900 focus:ring-2 focus:ring-[#0a5554] focus:border-[#0a5554]">
        </div>

        <div>
            <label for="subtitle" class="block text-sm font-semibold text-gray-700 mb-1">Organization (optional)</label>
            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $member->subtitle) }}" maxlength="255"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-900 focus:ring-2 focus:ring-[#0a5554] focus:border-[#0a5554]">
        </div>

        <div>
            <label for="linkedin_url" class="block text-sm font-semibold text-gray-700 mb-1">LinkedIn URL (optional)</label>
            <input type="text" name="linkedin_url" id="linkedin_url" value="{{ old('linkedin_url', $member->linkedin_url) }}" maxlength="512"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-gray-900 focus:ring-2 focus:ring-[#0a5554] focus:border-[#0a5554]">
        </div>

        <div>
            <label for="sort_order" class="block text-sm font-semibold text-gray-700 mb-1">Sort order</label>
            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $member->sort_order) }}" required min="0" max="99999"
        ...
        <div>
            <label for="photo" class="block text-sm font-semibold text-gray-700 mb-1">Photo</label>
            @if ($member->photoUrl())
                <p class="text-sm text-gray-600 mb-2">Current:</p>
                <img src="{{ $member->photoUrl() }}" alt="" class="h-24 w-24 rounded-full object-cover border border-gray-200 mb-3">
            @endif
            ...
            <p class="mt-1 text-xs text-gray-500">Leave empty to keep the current image. Max 5&nbsp;MB.</p>
        </div>
